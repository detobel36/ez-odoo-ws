...
/*    val course=Course()
    ...
    course.session_ids=new java.util.ArrayList[Int](){{add(1)}}
   ...
   val session=Session()
    ...
    session.course_id=1
    session.active=true
    session.attendee_ids=Array(1)
    val sId=ws.record.create(session,classOf[Session],uid)
    val sessions = ws.record.page(1, 5, List(), classOf[Session], uid)
  print()
  }
...
case class Course() extends OdooModel {
  ...
  @BeanProperty @OManyToOne var responsible_id: Int = _
  @BeanProperty var session_ids: java.util.ArrayList[Int] = _
}
...
case class Session() extends OdooModel {
  ...
  @BeanProperty @OManyToOne  var instructor_id: Int = _
  @BeanProperty @OManyToOne  var course_id: Int = _
  @BeanProperty var attendee_ids: Array[Int] = _
